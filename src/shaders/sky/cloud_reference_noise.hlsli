...
// the reference's 2D/3D normalized fBM and 2D Smooth F1.
#pragma once
...
float2 cloudEvolve2(float2 delta, uint seed, float time, uint cacheRow)
{
    if (time == 0.f) return delta;
    float s, c;
#if CLOUD_USE_NOISE_CACHE
    const float2 rotation = cloudCacheLoad(uint2(seed & 1023u, cacheRow));
    s = rotation.x; c = rotation.y;
#else
...
{
    delta.xy = cloudEvolve2(delta.xy,seed,time,1u);
    delta.yz = cloudEvolve2(delta.yz,seed >> 10u,time,1u);
    return delta;
}
float cloudPerlin2(float2 p, float time)
{
    ...
    {
        int2 o = int2(x,y);
        uint h = cloudHash2(uint2((cell+o)));
        float2 weight = lerp(1.f-w,w,float2(o));
        result += cloudGradient2(h,cloudEvolve2(f-float2(o),h,time,0u))*weight.x*weight.y;
    }
    return result*0.6616f;
}
float cloudPerlin3(float3 p, float time)
{
...
float cloudFbm2(float2 p, float2 offset, float detail, float roughness, float time)
{
    ...
    [loop] for (; octave<=int(detail); ++octave)
    {
        ...
        sum += amplitude*cloudPerlin2((p+offset)*scale,time);
        total += amplitude;
        amplitude *= roughness;
    }
    ...
    {
        const float scale = float(1u << octave);
        float next = cloudPerlin2((p+offset)*scale,time);
...
float2 cloudReferenceWarp(float2 p, float frequency=2.5f, float detail=2.f, float roughness=0.5f, float time=0.f)
{
    ...
    return float2(cloudFbm2(p*frequency,0.f,detail,roughness,time),
                  cloudFbm2(p*frequency,offset,detail,roughness,time));
}
float cloudReferenceFine(float3 p, float frequency=30.f, float detail=2.f, float roughness=0.5f, float time=0.f)
{
    ...
    [loop] for (; octave<=int(detail); ++octave)
    {
        sum += amplitude*cloudPerlin3(p*(frequency*float(1u << octave)),time);
        total += amplitude;
        amplitude *= roughness;
    }
    float value = sum/total;
    if (frac(detail) > 0.f)
    {
        float next = cloudPerlin3(p*(frequency*float(1u << octave)),time);
        value = lerp(value,(sum+amplitude*next)/(total+amplitude),frac(detail));
    }
    return 0.5f + 0.5f*value;
}
...
{
    float2 random = float2(cloudFloatHash(cell), float(cloudHash3(asuint(float3(cell,1.f)))) / 4294967295.f);
    if (time != 0.f)
    {
        const float2 rate = 0.5f + float2(cloudFloatHash(cell + 17.3f), cloudFloatHash(cell + 91.7f));
...
float cloudSmoothF1(float2 p, float smoothness=1.f, float randomness=2.f/3.f, float time=0.f)
{
    ...
    {
        int2 o = int2(x,y);
        float2 c = float2((cell+o));
        float2 random;
#if CLOUD_USE_NOISE_CACHE
        const int2 cacheCell = int2(c) - cloudCacheOrigin();
        if (all(cacheCell >= 0) && all(cacheCell < 128))
        {
            const uint index = uint(cacheCell.y * 128 + cacheCell.x);
            random = cloudCacheLoad(uint2(index % 1024, 2 + index / 1024));
        }
        else
#endif
            random = cloudVoronoiPoint(c, time);
