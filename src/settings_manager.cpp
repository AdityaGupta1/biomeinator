...
#define ADD_OPTION(name, desc, type, defaultValue) optionAdder(name, desc, cxxopts::value<type>()->default_value(defaultValue))
    ...
    ADD_OPTION("restirTemporalConfidenceCap", "ReSTIR PT confidence cap on the temporal neighbor (0 disables its contribution)", uint32_t, "8");
    ...
    ADD_OPTION("restirInitialBlend", "ReSTIR PT: weight of the pixel's own initial sample in the shading (0 = resampled only, 1 = initial sampling only); unbiased, restores per-frame noise for the denoiser", float, "0.5");
    ADD_OPTION("restirWhiteNoise", "ReSTIR PT: amplitude of zero-mean multiplicative white noise added before DLSS (0 disables); biased toward nothing in expectation but not an estimator", float, "0.2");
...
#define COPY_SETTING(name, type) settings[name] = parseResult[name].as<type>()
    ...
    COPY_SETTING("restirInitialBlend", float);
    COPY_SETTING("restirWhiteNoise", float);
