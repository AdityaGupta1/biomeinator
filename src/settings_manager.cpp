...
#define ADD_OPTION(name, desc, type, defaultValue)                                                                     \
    ...
    ADD_OPTION("cloudCoverage", "Cloud coverage", float, "0.3");
    ADD_OPTION("cloudDensity", "Cloud extinction per meter", float, "0.025");
    ADD_OPTION("cloudSteps", "Cloud view ray march steps", uint32_t, "32");
    ADD_OPTION("cloudBaseHeight", "Cloud base (m)", float, "650");
    ADD_OPTION("cloudThickness", "Layer thickness (m)", float, "450");
    ADD_OPTION("cloudPeriod", "Weather tile size (m)", float, "4096");
    ADD_OPTION("cloudMaxDistance", "Cloud draw distance (m)", float, "30000");
    ADD_OPTION("cloudWeatherScale", "Large shape frequency", uint32_t, "1");
    ADD_OPTION("cloudBillowScale", "Billow frequency", uint32_t, "3");
    ADD_OPTION("cloudPerlinWeight", "Perlin / cellular blend", float, "0.55");
    ADD_OPTION("cloudShapeThreshold", "Billow threshold", float, "0.2");
    ADD_OPTION("cloudShapeGain", "Billow contrast", float, "2.5");
    ADD_OPTION("cloudWeatherSoftness", "Weather edge softness", float, "0.025");
    ADD_OPTION("cloudBottomFade", "Bottom softness", float, "0.1");
    ADD_OPTION("cloudTopStart", "Top fade start", float, "0.55");
    ADD_OPTION("cloudTopVariance", "Top height variation", float, "0.35");
    ADD_OPTION("cloudMediumRepeats", "Medium detail frequency", uint32_t, "4");
    ADD_OPTION("cloudFineRepeats", "Fine detail frequency", uint32_t, "16");
    ADD_OPTION("cloudMediumErosion", "Medium erosion strength", float, "0.16");
    ADD_OPTION("cloudFineErosion", "Fine erosion strength", float, "0.04");
    ADD_OPTION("cloudWarpStrength", "Fine detail distortion", float, "0.4");
    ADD_OPTION("cloudAmbient", "Sky ambient strength", float, "0.3");
...
#define COPY_SETTING(name, type) settings[name] = parseResult[name].as<type>()
    ...
    COPY_SETTING("cloudWeatherScale", uint32_t);
    COPY_SETTING("cloudBillowScale", uint32_t);
    COPY_SETTING("cloudPerlinWeight", float);
    COPY_SETTING("cloudShapeThreshold", float);
    COPY_SETTING("cloudShapeGain", float);
    COPY_SETTING("cloudWeatherSoftness", float);
    COPY_SETTING("cloudBottomFade", float);
    COPY_SETTING("cloudTopStart", float);
    COPY_SETTING("cloudTopVariance", float);
    COPY_SETTING("cloudMediumRepeats", uint32_t);
    COPY_SETTING("cloudFineRepeats", uint32_t);
    COPY_SETTING("cloudMediumErosion", float);
    COPY_SETTING("cloudFineErosion", float);
