...
#define ADD_OPTION(name, desc, type, defaultValue)                                                                     \
    ...
    ADD_OPTION("perfMoveSpeed", "Perf run: move the camera forward at this many blocks per second while measuring", float, "0");
...
#define COPY_SETTING(name, type) settings[name] = parseResult[name].as<type>()
    ...
    COPY_SETTING("maxBlasBuildsPerFrame", uint32_t);
    COPY_SETTING("waterAnimationDistance", int);
