...
void init()
{
    ...
    BLOCK_DATA_BY_NAME(DARKSHROOM) = { .uvs = { uvec2(2, 2) }, .type = BlockType::TRANSPARENT_CUTOUT, .shape = BlockShape::X_SHAPED };
    BLOCK_DATA_BY_NAME(DARKSHROOM_STEM) = { .uvs = { uvec2(29, 1) } };
    BLOCK_DATA_BY_NAME(DARKSHROOM_CAP) = { .uvs = { uvec2(31, 1) } };
    BLOCK_DATA_BY_NAME(OBSIDIAN) = { .uvs = { uvec2(3, 2) } };
    BLOCK_DATA_BY_NAME(BLUE_SPARKLES) = { .uvs = { uvec2(4, 2) } };
}
