...
constexpr uint32_t maxStructures = 512;
...
template<class T> void append(std::vector<char>& bytes, T value)
{
...
public:
    ...
    void bytes(void* destination, size_t size)
    {
        ...
        if (size != 0) file.read(static_cast<char*>(destination), static_cast<std::streamsize>(size));
    ...
    void finish() const { require(remaining == 0, "unexpected trailing region data"); }
};
...
void validateBlockStates(const std::vector<Block>& blocks, const std::unordered_map<uint32_t, uint8_t>& states)
{
    require(states.size() <= numChunkBlocks, "too many block states");
    for (const auto& [index, state] : states)
    {
        require(index < blocks.size() && state < blockFaceCount &&
...
bool write(const std::filesystem::path& path, const Region& region, uint32_t& outChunksWritten)
{
    ...
    try
    {
        require(isValidPosition(region.regionPos), "region coordinates out of range");
        std::vector<std::pair<uint16_t, const Chunk*>> chunks;
        for (uint16_t index = 0; index < region.chunks.size(); ++index)
        {
            const auto& chunk = region.chunks[index];
            if (chunk && chunk->getState() >= ChunkState::HAS_ALL_BLOCKS) chunks.emplace_back(index, chunk.get());
        }

        std::vector<char> bytes;
        append(bytes, magic);
        append(bytes, currentVersion);
        append(bytes, static_cast<int32_t>(region.regionPos.x));
        append(bytes, static_cast<int32_t>(region.regionPos.y));
        ...
        for (const auto& [index, chunk] : chunks)
        {
            ...
            for (Block block : blocks) require(block < Block::COUNT, "invalid block");
            for (Biome biome : biomes) require(biome < Biome::COUNT, "invalid biome");
            for (size_t word = 0; word < maskWords; ++word)
                require((air[word] & solid[word]) == 0, "overlapping terrain masks");
            validateBlockStates(blocks, chunk->getBlockStates());
            const glm::ivec2 expectedChunkPos = region.regionPosChunks +
                glm::ivec2(index % regionSideLength, index / regionSideLength);
            require(chunk->getChunkPos() == expectedChunkPos, "chunk in wrong region slot");
            const glm::ivec2 origin = expectedChunkPos * static_cast<int>(chunkSizeXZ);
            ...
            require(structures.size() <= maxStructures, "too many surface candidates");
            if (!structures.empty())
            {
                payload.clear();
        ...
        if (!FileUtil::writeAtomically(path, bytes)) return false;
        outChunksWritten = static_cast<uint32_t>(chunks.size());
        return true;
    }
                             ...
                             std::span<const Block> blockRemap)
{
    try
    {
        ...
        for (Block block : blockRemap) require(block < Block::COUNT, "invalid block palette entry");
        ...
        auto region = std::make_unique<Region>(expectedPos);
        ...
        for (uint16_t i = 0; i < count; ++i)
        {
            ...
            for (Biome biome : data.biomes) require(biome < Biome::COUNT, "invalid biome");

            const glm::ivec2 chunkPos = region->regionPosChunks + glm::ivec2(index % regionSideLength, index / regionSideLength);
            ...
            if (compressedStructures != 0)
            {
                ...
                require(structureCount <= maxStructures && candidates.size() == sizeof(uint32_t) * (1 + structureCount),
            ...
            validateBlockStates(data.blocks, data.blockStates);
            ...
            if (version >= generationDataVersion)
            {
                ...
                for (uint32_t c = 0; c < caveCount; ++c)
                {
                    ...
                    data.caveStructures.push_back({ static_cast<CaveStructureType>(packed & 0xffu), position, static_cast<int>(height) });
                }
            }
            region->createChunk(chunkPos)->loadSerializedData(std::move(data));
        }
        reader.finish();
        return region;
    }
