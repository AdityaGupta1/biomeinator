...
void render()
{
    ...
    renderParams->cloud.period = std::clamp(SettingsManager::getAsFloat("cloudPeriod"), 1024.0f, 65536.0f);
    ...
    // Integer lattice periods at every octave prevent wrapping seams.
    renderParams->cloud.warpScale = std::round(renderParams->cloud.warpScale * 16.f) / 16.f;
    renderParams->cloud.fineScale = std::round(renderParams->cloud.fineScale * 16.f) / 16.f;
    ...
    renderParams->cloud.padding0 = renderParams->cloud.padding1 = 0.f;
