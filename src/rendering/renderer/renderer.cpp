...
void resize()
{
    ...
    renderState.dev_restirStats.Reset();
    renderState.dev_restirStats = BufferHelper::createBasicBuffer(
        RESTIR_STATS_COUNT * sizeof(uint32_t), &DEFAULT_HEAP, { .resourceFlags = D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS });
    renderState.dev_restirStats->SetName(L"dev_restirStats");
    renderState.dev_restirStatsReadback.Reset();
    renderState.dev_restirStatsReadback = BufferHelper::createBasicBuffer(RESTIR_STATS_COUNT * sizeof(uint32_t), &READBACK_HEAP);
    renderState.dev_restirStatsReadback->SetName(L"dev_restirStatsReadback");
                                ...
                                const uint32_t dispatchHeight)
{
    ...
    D3D12_DISPATCH_RAYS_DESC desc = useSharc ? renderState.sharc.queryDispatch : renderState.ptDispatchDesc;
    desc.RayGenerationShaderRecord.StartAddress = rtRaygenRecordAddress(
        useSharc ? renderState.sharc.queryShaderIds.Get() : renderState.dev_ptShaderIds.Get(), static_cast<uint32_t>(pass));
...
static void dispatchRestirReuse(ParamBlockManager& paramBlockManager)
{
    ...
    if (RESTIR_SHIFT_STATS && bool(paramBlockManager.restirParams->shiftStatsEnabled))
    {
        ...
        renderState.cmdList->CopyResource(renderState.dev_restirStatsReadback.Get(), renderState.dev_restirStats.Get());
        BufferHelper::stateTransitionResourceBarrier(renderState.cmdList.Get(), renderState.dev_restirStats.Get(),
...
void render()
{
        ...
        (!renderState.stopAccumulating || antialiasingMode != AntialiasingMode::ACCUMULATE))
    {
        ...
        // The SHaRC debug views are drawn by initial sampling and must not be shaded over by reuse
        const bool runRestirReuse = paramBlockManager.renderParams->samplingMode == static_cast<uint32_t>(SamplingMode::RESTIR_PT) &&
            restirShadesInResample && paramBlockManager.sharcParams->debugMode == 0;
        if (runRestirReuse)
        {
