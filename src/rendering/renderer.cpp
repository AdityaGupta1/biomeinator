...
void resize()
{
    ...
    const bool pathSplitting = SettingsManager::getAsBool("pathSplitting");
    const uint32_t pathTracingRawBufferSizeBytes =
        renderWidth * renderHeight * (pathSplitting ? 2 : 1) * sizeof(float) * 4;
...
static void imguiEndFrame(double deltaTime)
{
    ...
    if (ImGui::Begin("Settings", nullptr, windowFlags | ImGuiWindowFlags_AlwaysAutoResize))
    {
        ...
        needsResize |= SettingsGuiHelpers::Checkbox("Enable path splitting", "pathSplitting");
...
void render()
{
    ...
    const bool pathSplitting = SettingsManager::getAsBool("pathSplitting");
    renderParams->pathSplitting = pathSplitting ? 1 : 0;
    ...
    if (scene.hasTlas() && (!stopAccumulating || antialiasingMode != AntialiasingMode::ACCUMULATE))
    {
        ...
        ptDispatchDesc.Width = gbufferDispatchDesc.Width * (pathSplitting ? 2 : 1);
