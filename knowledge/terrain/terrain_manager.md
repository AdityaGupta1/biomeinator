...
`maxBlasBuildsPerFrame` (default 64) is the matching cap on the render side; at 8 it was the binding limit on generation time, and after the generator speedup 32 was again. The per-frame cap is proportional to the queue, an eighth of it clamped to [8, setting], so a load drains at the setting while a row of chunks becoming eligible while moving lands over several frames; 32 of them landing in one frame was the most frequent frame spike, and a hard threshold between the two rates would make a draining load visibly change speed.
