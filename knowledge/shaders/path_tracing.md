...
Fog and clouds are integrated per segment by `applySegmentAtmosphere`, on bounce segments as
well as the primary one. See [clouds](../rendering/clouds.md) for how the two media compose.
...
- **Water absorption** — `computeSegmentAbsorption` and `computePassthroughAbsorption` use `voxelBoundsMin/Max_WS` to compute water travel distance, which is only populated in voxel mode. The `PerFaceData` flag `FACE_FLAG_IS_WATER` is also never set outside voxel mode, so water-surface detection itself is voxel-only.
...
- **Air fog / god rays** — per-segment height fog with single-scattered sunlight, applied at the two segment-absorption call sites (skipped for underwater segments). Transmittance is closed-form; in-scattering marches jittered steps with one sun occlusion ray each, aimed at a fresh uniform sample of the sun cap rather than at its center (`light/fog.hlsli`) — an inline `RayQuery` that alpha-tests cutout candidates (deterministic 0.5 threshold, mip 0 — occlusion is boolean, so no ray cone or stochastic alpha) and does not occlude on perfectly specular transmitters (`isDeltaTransmission`, so water lets sun shafts through but rough glass does not) — plus an analytic `fogAmbientStrength * (1 - T) * avgSky` ambient term with no visibility check — it also brightens enclosed cave interiors, and the strength setting is the artistic control for that trade-off. Non-obvious rules:
  - The march weights the sun by `getAttenuatedSunIlluminance`, which is already `Le / pdf` for uniform cap sampling, so sampling the cap for the occlusion ray needs no extra sample weight — it only replaces a point-light visibility test with the area-light one the weighting always assumed. Phase and sun attenuation stay at the cap center: they vary negligibly over the cap and hoist out of the loop, and a zero attenuation (sun below the planet's horizon) skips the march entirely.
  - Each step is also attenuated by cloud transmittance towards the sun and along the view segment (`light/fog.hlsli`), which is what casts cloud shadows into god rays.
  - Only the material's scalar roughness factor decides delta transmission here. Resolving a roughness map (as the shadow passthrough in `path_tracing_common.hlsli` does) would cost a texture sample per candidate in the march's hot loop, so a specular texel under a rough factor occludes fog where it would pass a shadow ray. Erring towards occluding costs a wisp rather than leaking light.
